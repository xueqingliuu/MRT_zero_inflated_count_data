...
generate_data <- function(sample_size, total_T) {
  ...
  for (t in 1:total_T) {
    ...
    ### ZINB ###
    dta$prob_Y_A0[row_index] <- exp(baseline_Y_S0 + baseline_Y_S1 * dta$S[row_index])
...
if (0) {
  ...
  numerator <- exp(gam1 * 0.1) * exp(baseline_Y_S0) * exp(beta_0) + exp(gam1 * 1.1 + gam2) * exp(baseline_Y_S0 + baseline_Y_S1) * exp(beta_0 + beta_1) + exp(gam1 * 2.1 + 2 * gam2) * exp(baseline_Y_S0 + 2*baseline_Y_S1) * exp(beta_0 + 2 * beta_1)
  denominator <- exp(gam1 * 0.1) * exp(baseline_Y_S0) + exp(gam1 * 1.1) *  exp(baseline_Y_S0 + baseline_Y_S1) + exp(gam1 * 2.1) * exp(baseline_Y_S0 + 2*baseline_Y_S1)
