...
for (i_ss in 1:length(total_Ts)) {
  ...
  result <- foreach(isim = 1:nsim, .combine = "c") %dorng% {
    ...
    rand_prob_tilde = c(mean(dta$A1), mean(dta$A2))

    EMEE <- fit_EMEE(
      dta = dta,
      id_varname = "userid",
      decision_time_varname = "day",
      treatment_varname = c("A1", "A2"),
      outcome_varname = "Y",
    ...
    output <- list(list(EMEE = EMEE, EMEE_NonP = EMEE_NonP, DR_EMEE_NonP = DR_EMEE_NonP, 
                        GEE_IND = GEE_IND, GEE_EXCH = GEE_EXCH))
  }
  
  
  ee_names <- c("EMEE", "EMEE_NonP", "DR_EMEE_NonP", "GEE_IND", "GEE_EXCH")
  beta_names <- c("Intercept", moderator_vars)
  num_estimator <- length(ee_names)
  
  
  beta <- simplify2array(lapply(result, function(l) matrix(c(l$EMEE$beta_hat, l$EMEE_NonP$beta_hat,
  ...
  beta_se <- simplify2array(lapply(result, function(l) matrix(c(l$EMEE$beta_se, l$EMEE_NonP$beta_se, 
  ...
  result_df <- data.frame(time = rep(total_T, num_estimator),
                          ...
                          sd3 = result$sd[,3],
                          rmse3 = result$rmse[,3],
                          cp3 = result$coverage_prob[,3],
                          se3 = result$se[,3],
                          bias4 = result$bias[,4],
                          sd4 = result$sd[,4],
                          rmse4 = result$rmse[,4],
                          cp4 = result$coverage_prob[,4],
                          se4 = result$se[,4])
  names(result_df) <- c("ss", "est", "bias1", "sd1", "rmse1", "cp1", "se1",
                        "bias2", "sd2", "rmse2", "cp2", "se2",
                        "bias3", "sd3", "rmse3", "cp3", "se3",
                        "bias4", "sd4", "rmse4", "cp4", "se4")
  rownames(result_df) <- NULL
  result_df_collected <- rbind(result_df_collected, result_df)
}

saveRDS(result_df_collected, file = "result_simulation_mod_zinb_three.RDS")
