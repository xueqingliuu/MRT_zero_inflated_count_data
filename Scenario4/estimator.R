...
log_linear_GEE_geepack <- function(
  ...
  treatment_varname1 <- treatment_varname[1]
  treatment_varname2 <- treatment_varname[2]
  
  gee_formula <- as.formula(paste(outcome_varname, "~", control_summed, "+", treatment_varname1, moderator_summed, "+", treatment_varname2, moderator_summed))
  ...
  beta_hat <- fit_geepack$coefficients[(q+1):(q+2*p)]
  if (p > 1) {
    beta_hat[c(2,3)] <- beta_hat[c(3,2)]
  }
  ...
  names(beta_hat) <- names(beta_se) <- names(beta_se_adjusted) <- rep(Xnames, 2)
